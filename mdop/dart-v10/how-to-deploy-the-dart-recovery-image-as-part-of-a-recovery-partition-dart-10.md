...
當您執行完 Microsoft Diagnostics 與復原工具集（DaRT）10復原映射嚮導並建立復原映像之後，您可以從 ISO 映像檔中解壓縮 boot.ini 檔案，並將它部署為 Windows 10 影像中的復原分區。 建議使用分區，因為任何使 Windows 作業系統無法啟動的損壞問題，也會阻止復原映射的啟動。 個別的分區也不需要提供兩次 BitLocker 復原金鑰了。 考慮隱藏式磁碟分割，避免使用者在檔案上儲存檔案。

**在 Windows 10 影像的 [恢復] 分區中部署 DaRT**

1.  在您的 Windows 10 影像中建立一個等於或大於您使用**DaRT 10 [復原映像] 嚮導**所建立之 ISO 映像檔案大小的目標磁碟分割。
...
4.  使用 [恢復] 分區取代 Windows 10 影像中的目標分區。
